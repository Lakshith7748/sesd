...
export interface IBidRepository {
  ...
  findByFreelancerAndProject(freelancerId: string, projectId: string): Promise<IBid | null>;
  findByFreelancer(freelancerId: string): Promise<IBid[]>;
  save(data: Partial<IBid>): Promise<IBid>;
  update(id: string, data: Partial<IBid>, session?: ClientSession): Promise<IBid | null>;
}
