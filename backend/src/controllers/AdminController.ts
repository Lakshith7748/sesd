...
export class AdminController {
  ...
  openDispute = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const { projectId, reason } = req.body as { projectId: string; reason: string };
      const dispute = await this.adminService.openDispute(raisedBy, projectId, reason);
  ...
  resolveDispute = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const dispute = await this.adminService.resolveDispute(disputeId, resolution);
