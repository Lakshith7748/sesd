...
export class AdminService {
  ...
  async openDispute(raisedBy: string, projectId: string, reason: string): Promise<IDispute> {
    const project = await this.projectRepo.findById(projectId);
    if (!project) throw new AppError('Project not found', 404);

    const existing = await this.disputeRepo.findByProject(projectId);
    if (existing) throw new AppError('A dispute already exists for this project', 409);
