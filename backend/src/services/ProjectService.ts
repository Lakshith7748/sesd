...
export class ProjectService {
  ...
  async editProject(
    ...
    if (!project.canEdit())
      throw new AppError("Only OPEN projects can be edited", 400);
  ...
  async deleteProject(clientId: string, projectId: string): Promise<void> {
    ...
    if (!project.canDelete())
      throw new AppError("Only OPEN projects can be deleted", 400);
  ...
  async markCompleted(clientId: string, projectId: string): Promise<IProject> {
    ...
    if (
      ...
      throw new AppError(
        "Project must be ASSIGNED or IN_PROGRESS to complete",
        400,
      );
    }
