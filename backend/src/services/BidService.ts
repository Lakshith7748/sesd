...
export class BidService {
  ...
  async placeBid(freelancerId: string, projectId: string, dto: PlaceBidDTO): Promise<IBid> {
    const project = await this.projectRepo.findById(projectId);
    if (!project) throw new AppError('Project not found', 404);
    if (project.status !== ProjectStatus.OPEN)
      throw new AppError('Project is not accepting bids', 400);

    const existing = await this.bidRepo.findByFreelancerAndProject(freelancerId, projectId);
    if (existing) throw new AppError('You have already placed a bid on this project', 409);
  ...
  async getBidsForProject(clientId: string, projectId: string): Promise<IBid[]> {
    const project = await this.projectRepo.findById(projectId);
    if (!project) throw new AppError('Project not found', 404);
